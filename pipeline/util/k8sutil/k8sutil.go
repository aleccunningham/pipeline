...
const (
	// agentClientPort is the client port on client service and etcd nodes.
	agentClientPort = 2379

	agentVolumeMountDir      = "/var/agent"
	dataDir                  = etcdVolumeMountDir + "/data"
	etcdVersionAnnotationKey = "agent.version"
	peerTLSDir               = "/etc/etcdtls/member/peer-tls"
	peerTLSVolume            = "member-peer-tls"
	serverTLSDir             = "/etc/agent/member/server-tls"
	serverTLSVolume          = "member-server-tls"
	operatorEtcdTLSDir       = "/etc/agenttls/operator/agent-tls"
	...
	AnnotationScope = "agent.cncd.io/scope"
...
func GetAgentVersion(pod *v1.Pod) string {
	return pod.Annotations[agentVersionAnnotationKey]
}

func SetAgentVersion(pod *v1.Pod, version string) {
	pod.Annotations[agentVersionAnnotationKey] = version
}
...
func CreateClientService(kubecli kubernetes.Interface, clusterName, ns string, owner metav1.OwnerReference) error {
	ports := []v1.ServicePort{{
		...
		TargetPort: intstr.FromInt(AgentClientPort),
...
func createService(kubecli kubernetes.Interface, svcName, clusterName, ns, clusterIP string, ports []v1.ServicePort, owner metav1.OwnerReference) error {
	svc := newAgentServiceManifest(svcName, clusterName, clusterIP, ports)
...
func AddAgentVolumeToPod(pod *v1.Pod, pvc *v1.PersistentVolumeClaim) {
...
func NewSeedMemberPod(clusterName string, ms etcdutil.MemberSet, m *etcdutil.Member, cs api.ClusterSpec, owner metav1.OwnerReference, backupURL *url.URL) *v1.Pod {
	token := uuid.New()
	pod := newAgentPod(m, ms.PeerURLPairs(), clusterName, "new", token, cs)
...
// NewAgentPodPVC create PVC object from agent pod's PVC spec
func NewAgentPodPVC(m *etcdutil.Member, pvcSpec v1.PersistentVolumeClaimSpec, clusterName, namespace string, owner metav1.OwnerReference) *v1.PersistentVolumeClaim {
...
func newAgentPod(m *etcdutil.Member, initialCluster []string, clusterName, state, token string, cs api.ClusterSpec) *v1.Pod {
